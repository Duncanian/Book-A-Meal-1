...
class Menu(db.Model):
    ...
    def create_menu(cls, menu_option):
        ...
        return make_response(jsonify({
            "message" : "menu option with that name already exists"}), 400)
    ...
    def delete_menu(menu_id):
        ...
        db.session.commit()
        return make_response(jsonify({
            "message" : "menu option has been successfully deleted"}), 200)
...
class Order(db.Model):
    ...
    def create_order(cls, order_item, client_id, client_email):
        ...
        new_order = cls(order_item=order_item, price=in_menu.price,
